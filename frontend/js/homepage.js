/* homepage.js - updated to fetch user profile and populate profile fields + skills
   - Uses existing backend endpoint: GET http://localhost:5000/api/profile
   - Expects JWT in localStorage key 'token' and sends it as 'x-auth-token'
   - Preserves existing functions and ids; adds fetchUserProfile()
*/

// keep existing globals / state
let currentSection = 'home';
let editMode = false;
let navigationHistory = [];

// default (fallback) userData — will be overwritten by fetchUserProfile() if available
const userData = {
    name: 'Divya Sharma',
    ...
    // detailed versions (when fetched)
    skillsOfferedDetailed: [],
    skillsToLearnDetailed: []
};
...
const chatData = {
  ...
  'john': {
    name: 'John Smith',
    avatar: 'https://images.unsplash.com/photo-1472099645785-5658ab4ff4e?w=50&h=50&fit=crop&crop=face',
...
// ---------- New: fetch profile from backend and populate UI ----------
async function fetchUserProfile() {
  const token = getToken();
  if (!token) {
    console.warn('No token found in localStorage. Profile fields will show fallback/demo data.');
    // still call update display so UI shows defaults
    updateSkillsDisplay();
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // fallback: still update UI with defaults
      updateSkillsDisplay();
      updateProfileSkillsTags();
      return;
    }

    // Expected structure: payload.email, payload.phoneNumber, payload.profile (object)
    const profile = payload.profile || {};
    // Overwrite userData where available
    userData.name = profile.fullName || profile.fullname || userData.name;
    userData.username = profile.username ? `@${profile.username}` : (profile.username || userData.username);
    userData.email = payload.email || profile.email || userData.email;
    userData.phone = payload.phoneNumber || profile.phone || userData.phone;
    userData.location = profile.location || userData.location;
    userData.bio = profile.bio || userData.bio;

    // socialLinks may be inside profile.socialLinks
    const social = profile.socialLinks || {};
    const linkedinVal = social.linkedin || profile.linkedin || '';
    const portfolioVal = social.github || profile.github || profile.portfolio || '';
    ...
    if (locationEl) locationEl.value = userData.location || '';

    const bioEl = document.getElementById('bio');
    if (bioEl) bioEl.value = userData.bio || '';

    // social inputs (these in your HTML are likely 'input[name="linkedin"]' and 'input[name="portfolio"]')
    const linkedinInput = document.querySelector('input[name="linkedin"]');
    if (linkedinInput) linkedinInput.value = linkedinVal || '';

    const portfolioInput = document.querySelector('input[name="portfolio"]');
    if (portfolioInput) portfolioInput.value = portfolioVal || '';

    // Left side display name & username
    const displayNameEl = document.getElementById('displayName') || document.querySelector('.profile-card h3') || document.querySelector('.profile-display-name');
    if (displayNameEl) displayNameEl.textContent = userData.name || '';

    const usernameEl = document.querySelector('.username') || document.getElementById('userName') || document.getElementById('displayUsername');
    if (usernameEl) {
      // prefer payload username, otherwise keep fallback
      if (profile.username) usernameEl.textContent = `@${profile.username}`;
      else usernameEl.textContent = userData.username || '';
    }

    // Parse skills: handle multiple possible shapes stored in DB
    const rawOffered = payload.skillsOffered || profile.skillsOffered || profile.skills || [];
    const rawToLearn = payload.skillsToLearn || profile.skillsToLearn || profile.skillsToLearn || profile.skillsToLearn || profile.skillsWanted || payload.skillsToLearn || [];

    // Normalize detailed arrays
    const normalizeSkillObj = item => {
      if (!item) return null;
      if (typeof item === 'string') return { skillName: item, level: '', category: [] };
      // if item has category string convert to array
      const categoryArr = Array.isArray(item.category) ? item.category : (item.category ? [item.category] : []);
      return {
        skillName: item.skillName || item.name || item.skill || '',
        level: item.level || item.level || '',
        category: categoryArr
      };
    };

    userData.skillsOfferedDetailed = Array.isArray(rawOffered) ? rawOffered.map(normalizeSkillObj).filter(Boolean) : [];
    userData.skillsToLearnDetailed = Array.isArray(rawToLearn) ? rawToLearn.map(normalizeSkillObj).filter(Boolean) : [];

    // Simple arrays for other UI functions
    userData.teachingSkills = userData.skillsOfferedDetailed.map(s => s.skillName).filter(Boolean);
    userData.learningSkills = userData.skillsToLearnDetailed.map(s => s.skillName).filter(Boolean);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // fetch user's real profile and populate the profile tab
  fetchUserProfile();
});

function initializeApp() {
  // set left side display name safely (your HTML may have id 'displayName' or just text)
  const displayNameEl = document.getElementById('displayName') || document.querySelector('.profile-card h3');
  if (displayNameEl) displayNameEl.textContent = userData.name;

  // make sure username label exists
...
// ---------- rest of your existing functions (search, routing, chat, categories) ----------
// I preserved the logic and names; trimmed repeated code to keep this file focused.
// If your original file had more advanced implementations, keep them. Below are the
// functions necessary to let the profile part work and to update the skills UI.
...
function updateTimeGreeting() {
  ...
  if (greetingElement) greetingElement.textContent = `${greeting}, ${userData.name}!`;
}
...
function showSection(sectionName) {
  ...
  if (backButtonContainer) {
    backButtonContainer.style.display = (sectionName === 'home' || navigationHistory.length === 0) ? 'none' : 'block';
  }

  if (sectionName === 'messages') selectChat('aman');
  if (sectionName === 'skillCategory') populateSkillCategories();
}
...
/* Search, populateSkillCategories, showCategorySkills, sendSkillRequest, addSkill,
   updateSkillsDisplay, removeSkill, initializeMessages, selectChat, displayMessages,
   sendMessage functions should remain as in your original file.
   For brevity here I keep the implementations that update skills UI since they integrate with fetchUserProfile().
*/
...
// update dashboard skills and profile skills list
function updateSkillsDisplay() {
  // Teaching skills (dashboard)
...
function removeSkill(type, skillName) {
  ...
  if (type === 'teach') {
    userData.teachingSkills = userData.teachingSkills.filter(s => s !== skillName);
    userData.skillsOfferedDetailed = userData.skillsOfferedDetailed.filter(s => s.skillName !== skillName);
  } else {
    userData.learningSkills = userData.learningSkills.filter(s => s !== skillName);
    userData.skillsToLearnDetailed = userData.skillsToLearnDetailed.filter(s => s.skillName !== skillName);
  }
  updateSkillsDisplay();
}
...
// render the purple pill tags in profile page (Skills tab)
function updateProfileSkillsTags() {
  const teachingTagsContainer = document.getElementById('teachingSkillsTags') || document.getElementById('teachingSkills');
  const learningTagsContainer = document.getElementById('learningSkillsTags') || document.getElementById('learningSkills');

  if (teachingTagsContainer) {
    // show detailed info if available
    const html = (userData.skillsOfferedDetailed.length ? userData.skillsOfferedDetailed.map(s => {
      const cats = (s.category && s.category.length) ? ` — ${s.category.join(', ')}` : '';
      const level = s.level ? ` (${s.level})` : '';
      return `<span class="skill-tag">${escapeHtml(s.skillName)}${level}${cats} <button class="remove-skill" onclick="removeSkill('teach','${escapeAttr(s.skillName)}')">&times;</button></span>`;
    }).join('') : userData.teachingSkills.map(s => `<span class="skill-tag">${escapeHtml(s)} <button class="remove-skill" onclick="removeSkill('teach','${escapeAttr(s)}')">&times;</button></span>`).join(''));
    teachingTagsContainer.innerHTML = html;
  }

  if (learningTagsContainer) {
    const html = (userData.skillsToLearnDetailed.length ? userData.skillsToLearnDetailed.map(s => {
      const cats = (s.category && s.category.length) ? ` — ${s.category.join(', ')}` : '';
      const level = s.level ? ` (${s.level})` : '';
      return `<span class="skill-tag">${escapeHtml(s.skillName)}${level}${cats} <button class="remove-skill" onclick="removeSkill('learn','${escapeAttr(s.skillName)}')">&times;</button></span>`;
    }).join('') : userData.learningSkills.map(s => `<span class="skill-tag">${escapeHtml(s)} <button class="remove-skill" onclick="removeSkill('learn','${escapeAttr(s)}')">&times;</button></span>`).join(''));
    learningTagsContainer.innerHTML = html;
  }
}
...
// small helpers for safety when injecting text
...
/* The rest of your original functions (search suggestions, messaging, category listing etc.)
   should remain unchanged. If you want I can paste the entire original homepage.js content here
   merged with the fetchUserProfile changes, but I left the rest intact to avoid accidental logic changes.
*/
